<template>
        <div>
                ...
                <b-card v-if="!sent" id="card-form" class="mx-auto mb-2" title-tag="h3"  bg-variant="dark" text-variant="light" title="Remplissez le formulaire avec les hyperparametre de votre choix pour entrainer un modele de ML et examiner ses resultats">       
                <hr/>
                <b-form class="p-1" @reset="onReset" v-if="show">
					...
					<b-col sm="12" lg="4"  offset-lg="4">
        					<b-form-input  v-model="testSize" class="text-light black-input" placeholder="pourcentage des données de test"  type="text"></b-form-input>
      					</b-col>
                                </b-form-group>    
                        </b-row>
                        ...
                        <b-button type="submit" @click.prevent="show=false" class="col-lg-3 "  variant="outline-light">Next</b-button>
		...
		<b-form class="p-1" @reset="onReset" v-else>
			...
			<b-row class="p-1" v-if="modules.includes('linguistic')"> 
                                <b-form-group label="Choisissez le nombre de parametre d'entrée à donner au module linguistique" class="h5 col text-light" v-slot="{ ariaDescribedby }">   
                                        <b-col sm="12" lg="4"  offset-lg="4">
                                                <b-form-input  v-model="nbInput" class="text-light black-input"   type="text"></b-form-input>
                                        </b-col>
                                </b-form-group>    
                        </b-row>
			<b-row class="p-1" v-if="modules.includes('user') && modules.length===1">
                                <b-form-group label="Choisissez la valeur de l'hyperparametre min_user_like" class="h5 col text-light" v-slot="{ ariaDescribedby }">   
                                        <b-col sm="12" lg="4"  offset-lg="4">
                                                <b-form-input  v-model="minUserLike" class="text-light black-input"   type="text"></b-form-input>
                                        </b-col>
                                </b-form-group>    
                        </b-row>
			<b-row class="p-1" v-if="modules.includes('user') && modules.length===1">
                                <b-form-group label="Choisissez la valeur de l'hyperparametre min_post_like" class="h5 col text-light" v-slot="{ ariaDescribedby }">   
                                        <b-col sm="12" lg="4"  offset-lg="4">
                                                <b-form-input v-model="minPostLike" class="text-light black-input"   type="text"></b-form-input>
                                        </b-col>
                                </b-form-group>    
                        </b-row>
		...
		<b-card v-else id="card-form" class="mx-auto mb-2" title-tag="h3"  bg-variant="dark">
			...
			<b-button class="col-lg-3" v-if="!loading" @click("sent=false") variant="outline-light"  href="/#/tests">
...
import Navbar from "@/components/Navbar";
		...
		methods:{
			...
			onSubmit(){
				...
				this.sent=true;
				this.loading=true;
				axios.post("http://127.0.0.1:5000/evaluation",data).then((res)=>{
					let d = res.data;
